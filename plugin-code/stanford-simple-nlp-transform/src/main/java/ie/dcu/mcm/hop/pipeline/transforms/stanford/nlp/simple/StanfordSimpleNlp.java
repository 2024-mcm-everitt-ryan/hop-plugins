...
import static ie.dcu.mcm.hop.pipeline.transforms.stanford.nlp.simple.ParsedSentence.Builder.parsedSentenceBuilder;
...
public class StanfordSimpleNlp extends BaseTransform<StanfordSimpleNlpMeta, StanfordSimpleNlpData> {
    ...
    public boolean processRow() throws HopException {
        ...
        if (first) {
            ...
            sentenceBuilder = parsedSentenceBuilder()
                    .includePartOfSpeech(meta.isIncludePartOfSpeech());
        ...
        try {
            ...
            if (getTransformMeta().isDoingErrorHandling()) {
                ...
                logError(
                        BaseMessages.getString(PKG, "StanfordSimpleNlp.ErrorInTransformRunning") + e.getMessage());
                setErrors(1);
                stopAll();
    ...
    private void cacheIndexPositions() throws HopException {
        ...
        for (String field : data.sentenceFields) {
            ...
            if (data.indexOfMap.getOrDefault(field, -1) < 0) {
                ...
                if (data.indexOfMap.getOrDefault(field, -1) < 0) {
                    ...
                    throw new HopException(
                            BaseMessages.getString(
                                    PKG, "DetectLanguage.Exception.CouldnotFindField", field));
                }
            }
        }
        ...
        if (meta.isIncludePartOfSpeech()) {
            ...
            for (PennTreebankPartOfSpeech e : PennTreebankPartOfSpeech.values()) {
                String field = "penn_treebank_pos_" + e.name();
                if (data.indexOfMap.getOrDefault(field, -1) < 0) {
                    data.indexOfMap.put(field, data.outputRowMeta.indexOfValue(field));
                    if (data.indexOfMap.getOrDefault(field, -1) < 0) {
                        // The field is unreachable!
    ...
    private void processSentence(ParsedSentence sentence) throws HopTransformException {
        ...
        Object[] outputRow = new Object[sentence.getInputRow().length + data.indexOfMap.size() + OVER_ALLOCATE_SIZE];
        arraycopy(sentence.getInputRow(), 0, outputRow, 0, sentence.getInputRow().length);

        outputRow[data.indexOfMap.get(meta.getOutputFieldNamePrefix() + "text")] = sentence.getSentenceText();
        outputRow[data.indexOfMap.get(meta.getOutputFieldNamePrefix() + "index")] = sentence.getIndex();
        outputRow[data.indexOfMap.get(meta.getOutputFieldNamePrefix() + "index_start")] = sentence.getIndexBegin();
        outputRow[data.indexOfMap.get(meta.getOutputFieldNamePrefix() + "index_end")] = sentence.getIndexEnd();
        outputRow[data.indexOfMap.get(meta.getOutputFieldNamePrefix() + "character_count")] = sentence.getCharacterCount();
        outputRow[data.indexOfMap.get(meta.getOutputFieldNamePrefix() + "word_count")] = sentence.getWordCount();

        if (meta.isIncludePartOfSpeech()) {
            outputRow[data.indexOfMap.get(meta.getOutputFieldNamePrefix() + "pos_tagged")] = join(sentence.getSentencePosTags(), ",");
            outputRow[data.indexOfMap.get(meta.getOutputFieldNamePrefix() + "pos_tags")] = sentence.getSentenceTextPosTagged();
            ...
            for (PennTreebankPartOfSpeech tag : PennTreebankPartOfSpeech.values()) {
                ...
                outputRow[data.indexOfMap.get(field)] = (long) sentence.getSentencePosBag().getCount(tag.name());
            }
        }
