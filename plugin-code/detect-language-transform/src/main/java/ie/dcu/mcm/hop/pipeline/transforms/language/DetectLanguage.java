...
package ie.dcu.mcm.hop.pipeline.transforms.language;
...
public class DetectLanguage extends BaseTransform<DetectLanguageMeta, DetectLanguageData> {
    ...
    private final LanguageDetector detector = fromAllSpokenLanguages().withPreloadedLanguageModels().build();
    ...
    public boolean processRow() throws HopException {
        ...
        try {
            ...
            if (sendToErrorRow) {
                ...
                putError(
                        getInputRowMeta(), r, 1, errorMessage, meta.getCorpusField(), "DetectLanguage001");
            }
        }
    ...
    private void cacheIndexPositions() throws HopException {
        ...
        if (data.indexOfDetectedLanguageConfidence < 0) {
            data.indexOfDetectedLanguageConfidence = data.outputRowMeta.indexOfValue("detected_language_confidence");
            if (data.indexOfDetectedLanguageConfidence < 0) {
                // The field is unreachable!
                throw new HopException(
                        BaseMessages.getString(
    ...
    private void processSync(String corpus, Object[] inputRow) throws HopTransformException {
        ...
        if (!confidenceValues.isEmpty()) {
            ...
            if (confidenceValues.size() == 1) {
                ...
                Double secondMostLikelyLanguageProbability = new ArrayList<>(confidenceValues.values()).get(1);
                boolean eq = Objects.equals(mostLikelyLanguageProbability, secondMostLikelyLanguageProbability);
                boolean unk = (mostLikelyLanguageProbability - secondMostLikelyLanguageProbability) < detector.getMinimumRelativeDistance$lingua();
                if (!eq && !unk) {
                    detectedLanguage = mostLikelyLanguage;
                    confidence = mostLikelyLanguageProbability;
                }
